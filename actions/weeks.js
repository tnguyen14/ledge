...
import { getWeekId, getWeekStart, getWeekEnd, getWeekStartFromWeekId } from '../selectors/week.js';
...
export function loadWeek({
  ...
  return async function loadWeekAsync(dispatch, getState) {
    ...
    const transactions = await getTransactions(token, getWeekStart({
...
export function showMore(ahead) {
  return function showMoreAsync(dispatch, getState) {
    ...
    const referenceWeekId = ahead == true ? visibleWeeks[0] : visibleWeeks.pop();
    const offset = ahead == true ? 1 : -1;
    const data = {
      weekId: getWeekId({
        date: getWeekStartFromWeekId({
          weekId: referenceWeekId
        }),
        offset
      })
    }; // show the week in UI
