...
import { getWeekStart, getWeekEnd, getWeekId, getMonthId, getWeeksDifference } from './week.js';
const getTransactions = state => state.transactions;
const getTransactionsByYears = createSelector(getTransactions, transactions => {
  const years = {};
...
export const getSortedTransactions = createSelector(getTransactions, transactions => {
  if (!transactions) {
    return;
  }
...
export const calculateSum = createSelector(getTransactions, transactions => {
  ...
  const expenses = transactions.filter(tx => tx.syntheticType == 'expense' && !tx.carriedOver);
  if (!expenses) {
    return 0;
  }
  return sum(expenses.map(t => t.amount));
});
export const calculateWeeklyAverage = createSelector(getNumWeeks, calculateSum, (numWeeks, sum) => {
  return sum / numWeeks;
});
export const getCurrentYearWeeklyAverage = createSelector(getTransactionsByYears, years => {
...
export const getWeekById = createSelector(getWeeks, getId, (weeks, weekId) => weeks[weekId] || {
  weekId,
  transactions: []
});
...
const getSearchParams = state => Object.entries(state.searchParams).filter(param => !!param[1]);
export const getSearchResult = createSelector(getSortedTransactions, getSearchParams, (transactions, searchParams) => {
  return transactions.filter(tx =>
  // has to match all the provided search params
  searchParams.every(([key, value]) => String(tx[key]).toLowerCase().includes(String(value).toLowerCase())));
});