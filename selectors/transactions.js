...
import { getYear, differenceInCalendarWeeks } from 'https://cdn.skypack.dev/date-fns@2';
...
export const getYearAverages = createSelector(getYears, years => {
  return Object.keys(years).map(year => {
    ...
    const numWeeks = differenceInCalendarWeeks(new Date(transactions[0].date), new Date(transactions[transactions.length - 1].date));
    return {
      numWeeks,
      transactions,
      year,
...
export const getWeeks = createSelector(getTransactions, transactions => {
  ...
  Object.keys(transactions).forEach(function processTransactionForWeek(id) {
    const transaction = transactions[id];
    addTransactionToWeek(weeks, transaction); // account for multi-week transaction
