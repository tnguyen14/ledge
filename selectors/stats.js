...
import sortBy from 'https://esm.sh/lodash.sortby';
import { sum } from '../util/calculate.js';
const getTransactions = state => state.transactions;
const getCategories = state => state.categories;
export const getCategoriesTotals = createSelector(getTransactions, getCategories, (transactions, categories) => {
  // calculate total for each category
  return categories.map(cat => {
    const categoryTransactions = transactions.filter(t => t.category == cat.slug);
    return {
      amount: sum(categoryTransactions.map(t => {
        if (!t.budgetSpan) {
...
*/
export const getMonthsCashflow = createSelector(getTransactions, getMonthsIds, (transactionsByMonths, monthsIds) => {
  const cashflow = {
    accounts: {
      debit: new Set(),
      credit: new Set()
  ...
  cashflow.months = monthsIds.reduce((allMonths, monthId) => {
    ...
    const debitTransactions = groupBy(sortBy(transactions.filter(t => t.debitAccount == selectedAccount), ['creditAccount']), 'creditAccount');
    const creditTransactions = groupBy(sortBy(transactions.filter(t => t.creditAccount == selectedAccount), ['debitAccount']), 'debitAccount');
