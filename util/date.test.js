import { utcToZonedTime, zonedTimeToUtc } from 'https://cdn.skypack.dev/date-fns-tz@1';
import { expect } from 'https://cdn.skypack.dev/chai';
import { setISODay, format } from 'https://cdn.skypack.dev/date-fns@2';
const TIMEZONE = 'America/Chicago';
const localTime8pm = new Date('2021-12-05 20:05'); // localTime8pm is, if local system is
...
describe('date-fns', () => {
  it('tz', () => {
    ...
    expect(utcTimeForLocal8pm.toISOString()).to.equal('2021-12-06T02:05:00.000Z'); // revert conversion

    expect(utcToZonedTime(utcTimeForLocal8pm, TIMEZONE)).to.deep.equal(localTime8pm);
  });
  it('tz - get ISO day in local timezone', () => {
    const sundayNyAfterMidnightUtc = '2021-05-03T03:04:00.000Z';
    expect(zonedTimeToUtc(setISODay(new Date(format(utcToZonedTime(sundayNyAfterMidnightUtc, TIMEZONE), 'yyyy-MM-dd HH:mm')), 1), TIMEZONE).toISOString()).to.equal('2021-04-27T03:04:00.000Z');
  });
  it('format', () => {
