...
import { getDate, getWeekStart, getWeekEnd, getWeeksDifference } from '../selectors/week.js';
  ...
  switch (transaction.type) {
    ...
    case 'deposit':
      return {
        syntheticType: 'transfer',
        debitAccount: 'cash',
        creditAccount: transaction.category == 'side-job' ? 'side-job' : slugify(transaction.merchant)
      };
  }
}
...
function transformTransaction(transaction) {
  const budgetStartDate = transaction.budgetStart ? getDate({
    date: transaction.budgetStart
  }).toJSDate() : getWeekStart({
    date: transaction.date
  }).toJSDate();
  // use span as offset for budgetEnd
  const budgetEndDate = transaction.budgetEnd ? getDate({
    date: transaction.budgetEnd
  }).toJSDate() : getWeekEnd({
    date: transaction.date,
  ...
  return {
    ...{
      ...
      // TODO remove when items are migrated
...
export async function getTransactions(startDate, endDate) {
  ...
  console.log(`Received ${transactions.length} transactions for ${start} to ${end}`);
...
export async function getTransactionsWithMerchantName(merchant) {
  ...
  return await getJson(`${LEDGE_URL}/${listName}/items?${qs.stringify({
    where: [{
      field: 'merchant',
      op: '==',
      value: merchant
