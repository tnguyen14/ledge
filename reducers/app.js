import { LOAD_TRANSACTIONS, LOAD_TRANSACTIONS_SUCCESS, SET_FILTER, SET_DISPLAY_FROM, SET_TOKEN, REFRESH_APP } from '../actions/app.js';
...
export default function app(state = defaultState, action) {
  switch (action.type) {
    ...
    case LOAD_WEEK_SUCCESS:
      return { ...state,
        isLoading: false,
        weeksMeta: { ...state.weeksMeta,
          [action.data.weekId]: { ...state.weeksMeta[action.data.weekId],
            loaded: true
          }
        }
      };
    ...
    case SET_DISPLAY_FROM:
      const previousVisibleWeeks = getVisibleWeeks(state.weeksMeta); // show 4 weeks by default
      ...
      for (let week of previousVisibleWeeks) {
        newState.weeksMeta[week] = { ...newState.weeksMeta[week],
          visible: false
        };
      }
