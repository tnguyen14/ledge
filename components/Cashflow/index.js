...
import { useReactTable, flexRender, getCoreRowModel } from 'https://esm.sh/@tanstack/react-table@8.16.0';
import classnames from 'https://esm.sh/classnames@2';
import { usd } from 'https://esm.sh/@tridnguyen/money@1';
import { getPastMonthsIds, getWeekStartFromWeekId } from '../../selectors/week.js';
...
function Cashflow() {
  ...
  const years = useMemo(() => monthsIds.reduce((aggregate, monthId) => {
  ...
  const columns = useMemo(() => [{
    ...
    header: '',
    accessorKey: 'label'
  }].concat(Object.entries(years).reverse().map(([year, months]) => ({
    id: 'year',
    header: year,
  ...
  return /*#__PURE__*/React.createElement("div", {
    ...
    responsive: true
  }, /*#__PURE__*/React.createElement("thead", null, getHeaderGroups().map((headerGroup, j) => /*#__PURE__*/React.createElement("tr", {
    key: `header-group-${j}`
  }, headerGroup.headers.map(header => /*#__PURE__*/React.createElement("th", {
    key: `column-${header.index}`,
    colSpan: header.colSpan
  }, flexRender(header.column.columnDef.header, header.getContext())))))), /*#__PURE__*/React.createElement("tbody", null, getRowModel().rows.map((row, l) => {
    return /*#__PURE__*/React.createElement("tr", {
      key: `row-${l}`,
      className: classnames({
        highlight: highlightRows.includes(row.original.label)
      })
    }, row.getVisibleCells().map((cell, m) => /*#__PURE__*/React.createElement("td", {
      key: `cell-${m}`
