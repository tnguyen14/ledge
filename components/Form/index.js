...
import React, { useEffect, useRef, useCallback } from 'https://esm.sh/react@18.2.0';
...
import { submitTransaction, submitTransactionFailure, inputChange, resetForm } from '../../slices/form.js';
import { updateMerchantCounts, addRecurringTransaction, updateRecurringTransaction } from '../../slices/meta.js';
import { setSearchParams } from '../../slices/app.js';
import { updateTransactionSuccess, updateTransactionFailure, addTransactionSuccess, addTransactionFailure } from '../../slices/transactions.js';
import { SYNTHETIC_TYPES, decorateTransaction, getUniqueTransactionId } from '../../util/transaction.js';
import { postTransaction, patchTransaction, getTransactionsWithMerchantName } from '../../util/api.js';
import { addMerchantToCounts, removeMerchantFromCounts } from '../../util/merchants.js';
...
function Form() {
      ...
      if (action == 'update') {
        try {
          if (values.syntheticType == 'recurring') {
            dispatch(updateRecurringTransaction({
              id: values.id,
            ...
            if (decoratedTransaction.merchant != oldMerchant) {
              const transactionsWithOldMerchantName = await getTransactionsWithMerchantName(oldMerchant);
              const updatedMerchantsCount = addMerchantToCounts(removeMerchantFromCounts(merchants_count, oldMerchant, transactionsWithOldMerchantName.length), decoratedTransaction.merchant);
        ...
        try {
          if (values.syntheticType == 'recurring') {
            ...
            const id = await getUniqueTransactionId(new Date(decoratedTransaction.date).valueOf());
            ...
            const updatedMerchantsCount = addMerchantToCounts(merchants_count, decoratedTransaction.merchant);
            dispatch(updateMerchantCounts(updatedMerchantsCount));
          }
        } catch (e) {
          console.error(e);
  ...
  return /*#__PURE__*/React.createElement("form", {
    ...
    method: "POST"
  }, /*#__PURE__*/React.createElement(React.Fragment, null, fields.map(fieldConfig => {
    const field = {
      ...fieldConfig
    };
    ...
    if (field.name == 'calculate') {
      afterButton = /*#__PURE__*/React.createElement(ZapIcon, null);
    }
    if (field.name == 'budgetSpan') {
      afterButton = /*#__PURE__*/React.createElement(Span, null);
    }
    ...
    type: "submit",
    onClick: submitForm
  }, buttonAttrs), actionText), /*#__PURE__*/React.createElement(Button, _extends({
