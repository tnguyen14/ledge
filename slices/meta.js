import { createAsyncThunk, createSlice, isAnyOf } from 'https://esm.sh/@reduxjs/toolkit@2';
...
const pastYears = [...Array(currentYear - 2018).keys()].map(index => 2018 + index);
const initialState = {
  merchants: [],
  expenseCategories: [],
  merchants_count: {},
...
const getMerchantNamesFromMerchantCounts = createSelector(state => state, counts => Object.keys(counts).filter(merchant => {
...
export const updateMerchantCounts = createAsyncThunk('meta/updateMerchantCounts', async merchants_count => {
  await patchMeta({
    merchants_count
  });
  return merchants_count;
});
export const addRecurringTransaction = createAsyncThunk('meta/addRecurringTransaction', async (transaction, {
  getState
}) => {
  const {
    recurring
...
export const updateRecurringTransaction = createAsyncThunk('meta/updateRecurringTransaction', async (transaction, {
  getState
}) => {
  const {
    recurring
  } = getState().meta;
  const transactionIndex = recurring.findIndex(t => t.id === transaction.id);
...
export const removeRecurringTransaction = createAsyncThunk('meta/removeRecurringTransaction', async (transactionId, {
  getState
}) => {
  const {
    recurring
  } = getState().meta;
  const transactionIndex = recurring.findIndex(transaction => transaction.id === transactionId);
  const updatedRecurring = [...recurring.slice(0, transactionIndex), ...recurring.slice(transactionIndex + 1)];
  await patchMeta({
    recurring: updatedRecurring
  });
  return updatedRecurring;
...
const meta = createSlice({
  ...
  reducers: {
    loadMetaSuccess: (state, action) => {
      state.merchants = getMerchantNamesFromMerchantCounts(action.payload.merchants_count);
      state.expenseCategories = action.payload.expenseCategories;
      state.merchants_count = action.payload.merchants_count;
      state.accounts = [...builtinAccounts.map(acct => ({
        ...acct,
        builtIn: true
      })), ...(action.payload.accounts || [])];
      Object.keys(action.payload.stats).forEach(year => state.stats[year] = action.payload.stats[year]);
    ...
    removeAccount: (state, action) => {
      state.accounts = state.accounts.filter(acct => {
  ...
  extraReducers: builder => {
    builder.addCase(updateMerchantCounts.fulfilled, (state, action) => {
      state.merchants_count = action.payload;
      state.merchants = getMerchantNamesFromMerchantCounts(action.payload);
    }).addMatcher(isAnyOf(addRecurringTransaction.fulfilled, updateRecurringTransaction.fulfilled, removeRecurringTransaction.fulfilled), (state, action) => {
