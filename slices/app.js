...
import { loadTransactions, removingTransaction, removeTransactionSuccess } from './transactions.js';
import { updateUserSettings, updateUserSettingsSuccess, updateUserSettingsFailure, removeRecurringTransaction } from './meta.js';
const initialState = {
  appReady: false,
  isLoading: false,
  filter: '',
...
const app = createSlice({
  ...
  extraReducers: builder => {
    builder.addCase(loadTransactions.pending, state => {
      ...
      state.isLoading = false, state.loadedTransactions = true, state.notification = {
        title: 'App',
        content: `Finished loading transactions from ${action.payload.start.toLocaleString(DateTime.DATETIME_FULL)} to ${action.payload.end.toLocaleString(DateTime.DATETIME_FULL)}`,
        autohide: 3000
      };
    ...
    }).addMatcher(isAnyOf(removeTransactionSuccess, removeRecurringTransaction.fulfilled), state => {
      state.transactionRemovalIntended = false;
      state.waitingTransactionRemoval = false;
      state.transactionToBeRemoved = undefined;
    });
