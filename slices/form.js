...
import { addTransactionSuccess, addTransactionFailure, updateTransactionSuccess, updateTransactionFailure } from './transactions.js';
...
function getFormFields(syntheticType, isSearch) {
  if (isSearch) {
    return [amountField, searchAccountField, merchantField, categoryField, dateField, timeField, memoField, idField];
  }
  switch (syntheticType) {
    case 'expense':
      return [creditAccountField, debitAccountField, amountField, calculateField, merchantField, categoryField, dateField, timeField, budgetStartField, budgetEndField, budgetSpanField, memoField, syntheticTypeField, idField];
    case 'recurring':
      return [creditAccountField, debitAccountField, amountField, calculateField, merchantField, categoryField, dateField, timeField, recurrenceFrequencyField, recurrencePeriodField, recurrenceDay, recurrenceEndDateField, syntheticTypeField, idField];
    case 'transfer':
    default:
      return [creditAccountField, debitAccountField, amountField, calculateField, merchantField, dateField, timeField, memoField, syntheticTypeField, idField];
  }
}
...
const initialState = {
  ...
  recurrenceDays: getRecurrenceDayValues(initialValues.recurrencePeriod, new Date(`${initialValues.date} ${initialValues.time}`))
};
const form = createSlice({
  ...
  reducers: {
    ...
    inputChange: (state, action) => {
      ...
      if (action.payload.name == 'date') {
        ...
        state.values.budgetEnd = format(DateTime.fromJSDate(new Date(`${state.values.budgetStart} 00:00`)).plus({
          weeks: state.values.budgetSpan - 1
        }).toJSDate(), DATE_FIELD_FORMAT);
        const newDate = new Date(`${action.payload.value} ${state.values.time}`);
        state.recurrenceDays = getRecurrenceDayValues(state.values.recurrencePeriod, newDate);
        state.values.recurrenceDay = getRecurrenceDay(state.values.recurrencePeriod, newDate);
      }
      if (action.payload.name == 'budgetStart') {
        state.values.budgetEnd = format(DateTime.fromJSDate(new Date(`${action.payload.value} 00:00`)).plus({
          weeks: state.values.budgetSpan - 1
        }).toJSDate(), DATE_FIELD_FORMAT);
      }
      if (action.payload.name == 'budgetEnd') {
        state.values.budgetSpan = getWeeksDifference({
          dateStart: new Date(`${state.values.budgetEnd} 00:00`).toISOString(),
          dateEnd: new Date(`${state.values.budgetStart} 00:00`).toISOString()
        }) + 1;
      }
      if (action.payload.name == 'budgetSpan') {
        state.values.budgetEnd = format(DateTime.fromJSDate(new Date(`${state.values.budgetStart} 00:00`)).plus({
          weeks: action.payload.value - 1
        }).toJSDate(), DATE_FIELD_FORMAT);
      }
      ...
      if (action.payload.name == 'recurrencePeriod') {
        ...
        state.recurrenceDays = getRecurrenceDayValues(action.payload.value, newDate);
        state.values.recurrenceDay = getRecurrenceDay(action.payload.value, newDate);
      }
    },
    resetForm: state => {
      state.values = {
  ...
  extraReducers: builder => {
    builder.addCase(editTransaction, (state, action) => {
      state.action = 'update';
      state.values = {
        ...action.payload,
        amount: fromCents(action.payload.amount),
        date: action.payload.date ? format(new Date(action.payload.date), DATE_FIELD_FORMAT) : undefined,
        time: action.payload.date ? format(new Date(action.payload.date), TIME_FIELD_FORMAT) : undefined,
        budgetStart: action.payload.budgetStart ? format(new Date(action.payload.budgetStart), DATE_FIELD_FORMAT) : undefined,
        budgetEnd: action.payload.budgetEnd ? format(new Date(action.payload.budgetEnd), DATE_FIELD_FORMAT) : undefined,
        calculate: ''
      };
    ...
    }).addMatcher(isAnyOf(addTransactionSuccess, updateTransactionSuccess, addRecurringTransaction.fulfilled, updateRecurringTransaction.fulfilled), state => {
      state.pending = false;
      state.values = {
        ...createInitialValues(state.action == 'search'),
        ...getAccountsValues(state.values.syntheticType),
      ...
      state.action = 'add';
    }).addMatcher(isAnyOf(addTransactionFailure, updateTransactionFailure), state => {
      state.pending = false;
    });
  }
});
